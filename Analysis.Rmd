...
Before any downstream analysis, a thorough quality control of the data has to be performed to make sure no technical artifacts will influence the differential expression results.

## Quality Control

The quality of the raw read data is performed by the Sequencing Centre and available in the fastqc reports. In this QC, a closer look at the mapped reads and the sample similarity/differences is done.

After removing the transcripts without any reads mapped to them, 26579 transcripts remain for further analysis. To assess the depth of coverage, a visualization of the total number of reads mapped to the transcriptome per samples is shown in Figure \@ref(fig:barplot). There are no systematic differences between the conditions and most samples have between 20 and 30 million mapped reads (on average 25 million). This is sufficient for a proper differential experiment analysis.
...
knitr::include_graphics("./Report/QC/barplot_raw_counts.pdf")
```

We can further have a look at the distribution of the reads per transcript per sample before and after normalization (see Figure \@ref(fig:boxplots)). Normalization methods are standardly applied with the intent of reducing the non-biologically derived variability inherent in transcriptomic measurements, mainly as a consequence of the different sequencing depths per sample. In this dataset, small differences in distribution can be seen before normalization, although not very large and not systematic. After normalization, the distributions are very comparable between the samples. This is a requirement for a valid differential expression analysis. 
...
knitr::include_graphics(c("./Report/QC/boxplot_raw_counts.pdf","./Report/QC/boxplot_normalized_counts.pdf"))
```
...
knitr::include_graphics("./Report/QC/PCA_rlog_top100.pdf")
```

The figures of this QC can be found in the *QC/* folder in the *Report/* directory. They also include PCA plots for a different number of variable genes.
...
* Group1_Excl:
^[The *Excl* indicates that one sample has been removed for this comparison - as has been described in the metadata excel.]
...
Raw counts were processed using the R package DESeq2 (v1.30.1) as described in the vignette. GSEA was performed using the R package clusterProfiler (v3.18.1) and visualized using enrichplot (v1.10.2). The complete script to replicate the analysis is included in the result folder.
...
* The *QC/* folder contains figures describing the data quality and basic statistics.
...
The *tex_files/* folder contains configuration files for the proper rendering of the report and should not be modified.
